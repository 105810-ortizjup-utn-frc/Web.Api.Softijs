...
using Web.Api.Softijs.Seed.Helpers;
...
builder.Services.AddEntityFrameworkSqlServer().AddDbContext<SoftijsDevContext>(options => options.UseSqlServer("Persist Security Info=False;Data Source=2022-softijs-sql-server-aut.database.windows.net;User ID=softijs-web-api;Password=MeGustaElIceCream2022;Initial Catalog=2022-softijs-sql-db-aut"));
...
{
    var dbContext = scope.ServiceProvider.GetRequiredService<SoftijsDevContext>();
    Seed seedData = new Seed();
    seedData.SeedProvincias(dbContext);
    seedData.SeedCiudades(dbContext);
}
